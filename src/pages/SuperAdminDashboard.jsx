...
export default function SuperAdminDashboard() {
  ...
  const isError = false; // React Query handles errors per-hook
      ...
      <div className="main-content" style={{ marginLeft: sidebarOpen ? 220 : 64, flex: 1, padding: '32px', paddingTop: 96, transition: 'margin-left 0.2s ease', minHeight: '100vh' }}>
        ...
        {section === 'clients' && (
          <div>
            ...
            {tenants.length === 0 ? <div style={{ textAlign: 'center', padding: '60px 0', color: c.muted }}><p style={{ fontSize: 40, marginBottom: 16 }}>🌿</p><p>No clients yet.</p></div>
              : tenants.map(tenant => (
        ...
        {/* ════════ PROSPECTING ════════ */}
        {section === 'prospecting' && <ProspectingPanel currentUser={user} />}

        {/* ════════ AGENT STATS ════════ */}
        {section === 'agentstats' && isEBAgent && <AgentStatsPanel user={user} />}

        {/* ════════ EB TEAM ════════ */}
        {section === 'ebteam' && <EBTeamPanel isSuperAdmin={isSuperAdmin} tenants={tenants} onReload={refetch} />}
        ...
        {section === 'users' && (
          <div>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>All Users ({allUsers.length})</h3>
            {allUsers.map(u => (
