...
const statusConfig = {
  awaiting_menu: { label: 'Awaiting Reply', color: '#eab308' },
  escalated: { label: 'Escalated', color: '#ef4444' },
  closed: { label: 'Closed', color: '#22c55e' },
  capture_name: { label: 'Capturing Name', color: colors.lime },
  qualified: { label: 'Qualified', color: '#34d399' },
  not_qualified: { label: 'Not Qualified', color: '#ef4444' },
};
...
export default function Dashboard() {
  ...
  const getStatusStyle = (status) => statusConfig[status] || { label: status || 'Unknown', color: '#888' };

  const sendAgentReply = async () => {
    if (!selectedLead || !replyInput.trim()) return;

    try {
      await api.post(`/leads/${selectedLead._id}/message`, { message: replyInput });
      setReplyInput('');
      // Refresh leads
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: colors.text, padding: '100px 40px 80px' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
          ...
          <button onClick={() => navigate('/')} style={{ color: colors.muted }}>← Back to Home</button>
        </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '16px',
                    cursor: 'pointer',
                    border: selectedLead?._id === lead._id ? `2px solid ${colors.lime}` : 'none',
                  }}
                >
          ...
          <div style={{ background: colors.card, borderRadius: '24px', padding: '32px', border: `1px solid ${colors.border}` }}>
            {selectedLead ? (
              <>
                ...
                <div style={{ display: 'inline-block', padding: '6px 16px', background: getStatusStyle(selectedLead.workflowStatus).bg || '#222', color: getStatusStyle(selectedLead.workflowStatus).color, borderRadius: '999px', marginTop: '8px' }}>
                  {getStatusStyle(selectedLead.workflowStatus).label}
                </div>

                <div style={{ height: '380px', background: '#0A0A08', borderRadius: '16px', padding: '20px', marginTop: '20px', overflowY: 'auto' }}>
                  <p style={{ color: colors.muted, textAlign: 'center', marginTop: '80px' }}>
                    Real conversation history will appear here once backend endpoint is connected.
                  </p>
                </div>
                ...
                <div style={{ marginTop: '20px', display: 'flex', gap: '12px' }}>
                  <input
                    ...
                    placeholder="Type reply as agent..."
                    style={{ flex: 1, padding: '16px', background: '#1C1C19', border: `1px solid ${colors.border}`, borderRadius: '999px', color: colors.text }}
                  />
                  <button onClick={sendAgentReply} style={{ padding: '0 32px', background: colors.lime, color: '#050505', border: 'none', borderRadius: '999px', fontWeight: '700' }}>
                    Send
                  </button>
                </div>
              </>
